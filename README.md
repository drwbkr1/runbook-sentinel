...
Candidate `v0.0.18` adds a closed 17-code model-output failure taxonomy without accepting more output or changing authority. The frozen 19-case reveal is exact. A same-manifest comparison classifies all 75 rejected local-model outputs, but the candidate model still has diagnosis accuracy, benign utility, and `pass^3` of 0.0 and is excluded; deterministic control remains the default. Source, package, real-surface, clean-clone, exact GitHub review/merge, and fresh merged-main gates pass; final release audit, tag, assets, and public verification remain pending.
