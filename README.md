...
Latest verified public release: `v0.0.15`. Candidate `v0.0.16` adds a canonical per-event SHA-256 chain, refuses append to a corrupt prefix, and binds completed evaluations to the exact companion trace event count and final digest. Its source and packaged `84+9+6+10+10` gates, real MCP/API/executor/state/telemetry surfaces, and rendered dashboard pass locally; public release verification is still pending. The chain is unkeyed: it does not authenticate the writer or provide hostile-writer resistance, immutable storage, non-repudiation, or RFC 5848 conformance. Candidate `v0.0.11` remains rejected and unpublished with its stale rendered-label failure preserved. Public source or package availability does not imply production readiness or authorization to connect Runbook Sentinel to operational infrastructure.
...
```powershell
$env:PYTHONPATH = 'src'
python -m runbook_sentinel evaluate --output artifacts/evaluations/runs/baseline-0016-manual.json
...
python scripts/verify_package_contract.py --contract eval/package-contract-0016.json --archive dist/runbook-sentinel-0.0.16.pyz
