...
Release checkpoint `v0.0.28` is verified on merged public `main` and prepared for source/zipapp publication. It adds only fail-closed reporting that distinguishes 60 hostile-guidance attempts retrieved then filtered from 6 attempts where the declared hostile document was never retrieved; no retrieval behavior or authority changes. PR `#26` merged exact reviewed head `99ed8f96f55c994feff168252fcde0941054746b` with history preserved as `9aec054f8c97f04c36fc39685667e86c5e8d097e`. Fresh public-main source, zipapp, MCP, API, state, telemetry, dashboards, and local-container evidence pass; final audit commit `447cab724ec7ad32a5c041883e7ea6c50c34d51b` authorizes only the exact closure, annotated tag, selected zipapp/checksum, and immediate public verification. The image is not exported, pushed, or published; consult `docs/status.md` for live publication state.
