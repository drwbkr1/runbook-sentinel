...
Release checkpoint `v0.0.30` is verified on merged public `main` and prepared for source/zipapp publication. It adds one exact diagnosis-identifier pattern to the optional local-model JSON Schema; v3 improves syntax acceptance but remains `selected-experimental-remediate` because diagnosis accuracy, benign utility, pass-three reliability, proposals, and executions remain inadequate. `deterministic-control-v2` stays the product default. PR `#28` merged exact reviewed head `cef8b3f00ab33227539d8d0646937da302eb1d52` with history preserved as `053ff98d04cfabfc67c7e7fb08d1de10ba560a1b`; final audit commit `e5cc9f2cb75c6fc8cc20400eb0b286cbe44e0e05` verifies all 20 required source, package, model-selection, rendered, runtime, container, scan, clean-context, and public-byte surfaces. Only the selected zipapp and checksum are release assets. The image is not exported, pushed, or published; consult `docs/status.md` for live publication state.
