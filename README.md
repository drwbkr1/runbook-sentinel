...
Release candidate: `v0.0.6`, a synthetic-only research preview that explicitly grades evidence-condition coverage across development and held-out splits while retaining exact isolated approval/execution trajectories, terminal state, and the excluded local-model comparison. Public source availability does not imply production readiness or authorization to connect it to operational infrastructure.
...
```powershell
$env:PYTHONPATH = 'src'
python -m runbook_sentinel evaluate --output artifacts/evaluations/runs/baseline-0006-manual.json
...
## Evidence-condition and terminal-state evaluation

Baseline 0006 explicitly labels complete, incomplete, stale, conflicting, and instruction-bearing evidence and requires every condition plus adversarial coverage in both development and held-out splits. The selected deterministic candidate executes 15 expected actions through the real approval and executor path, proves 45 no-action trials remain unchanged, covers all three synthetic actions, rejects consumed-token replay, verifies postconditions, and grades audit and trace sequences separately. The harness runs only after the agent result exists, holds approval material only in disposable evaluation state, and is not exposed by the agent, CLI runtime, API automation, MCP, or dashboard.
