...
from scripts.verify_retrieval_tier_cap_result import (
    EXPECTED_SELECTION_MANIFEST_SHA256,
    _resolve_selection_manifest,
)
...
class BaselineTest(unittest.TestCase):
    ...
    def test_retrieval_selection_manifest_survives_release_manifest_renewal_exactly(self):
        root = Path(self.temp.name)
        active = root / "eval/manifest.json"
        archived = root / "artifacts/verification/baseline-0031-prebuild-source-manifest.json"
        active.parent.mkdir(parents=True)
        archived.parent.mkdir(parents=True)
        exact = (
            ROOT / "artifacts/verification/baseline-0031-prebuild-source-manifest.json"
        ).read_bytes()
        self.assertEqual(hashlib.sha256(exact).hexdigest(), EXPECTED_SELECTION_MANIFEST_SHA256)

        active.write_bytes(exact)
        errors: list[str] = []
        selected, selection_sha256, active_sha256 = _resolve_selection_manifest(
            active, archived, errors
        )
        self.assertEqual((selected, selection_sha256, active_sha256), (active,) + (EXPECTED_SELECTION_MANIFEST_SHA256,) * 2)
        self.assertEqual(errors, [])

        active.write_bytes(b'{"checkpoint":"baseline-0031","release":true}\n')
        ...
        self.assertEqual(selection_sha256, EXPECTED_SELECTION_MANIFEST_SHA256)
        self.assertNotEqual(active_sha256, EXPECTED_SELECTION_MANIFEST_SHA256)
        self.assertEqual(errors, [])

        archived.write_bytes(exact + b"tampered")
        errors = []
        selected, _, _ = _resolve_selection_manifest(active, archived, errors)
        self.assertEqual(selected, active)
        self.assertEqual(errors, ["selection_manifest_archive_identity"])

    def test_container_v14_build_command_is_frozen_and_local_only(self):
        tag = "runbook-sentinel:baseline-0035-test"
        ...
        self.assertEqual(
            ...
            "type=image,name=runbook-sentinel:baseline-0035-test,"
            "rewrite-timestamp=true,unpack=false,store=true,push=false",
        )
        self.assertEqual(SOURCE_DATE_EPOCH, "1788207258")
        self.assertEqual(SOURCE_DATE_EPOCH_UTC, "2026-08-31T20:14:18Z")
        self.assertEqual(EXPECTED_BUILDER["buildkit"], "0.29.0")
        self.assertIn('keeper = f"rs-b0035-{token}"', verifier_source)
        self.assertNotIn('keeper = f"rs-b0033-{token}"', verifier_source)
        self.assertIn("all 49 checks true", verifier_source)
        self.assertNotIn("all 43 checks true", verifier_source)

    def test_container_v14_prerequisite_requires_current_implementation_phase(self):
        contract = {
            "status": "pass",
            "implementation_phase": "implemented_v14",
        }
        release_identity = {
            "status": "pass",
            "phase": "implemented",
        ...
        self.assertTrue(result["checks"]["release_identity"])
        ...
        contract["implementation_phase"] = "implemented_v14"
        release_identity["candidate_selected"] = True
        with mock.patch(
            "scripts.verify_container_runtime.run",
            side_effect=[
                completed(contract),
            ...
            with self.assertRaisesRegex(AssertionError, '"release_identity": false'):
                validate_prerequisites(evaluation)
    ...
    def test_container_v4_local_content_identity_and_events_fail_closed(self):
        ...
        tags = ["runbook-sentinel:baseline-0035-a-test", "runbook-sentinel:baseline-0035-b-test"]
        events = [
            {
                "Action": "tag",
    ...
    def test_mcp_exposes_diagnostics_and_proposals_but_no_execution(self):
        ...
        self.assertEqual(initialized["result"]["serverInfo"]["version"], "0.0.35")
    ...
    def test_live_http_surface_has_security_headers_and_runs_scenario(self):
        ...
        try:
            ...
            with urlopen(f"http://127.0.0.1:{server.server_port}/health") as response:
                self.assertEqual(json.loads(response.read())["checkpoint"], "baseline-0035")
            with urlopen(
    ...
    def test_container_v14_verifier_requires_current_dashboard_checkpoint(self):
        ...
        self.assertIn('b"Baseline 0035" in dashboard_raw', runtime)
        self.assertNotIn('b"Baseline 0033" in dashboard_raw', runtime)
        self.assertIn("'b\"Baseline 0033\" in dashboard_raw' not in runtime_text", validator)

    def test_container_v14_retrieval_quality_projection_fails_closed(self):
        report = json.loads(
            (
                ROOT
                / "artifacts/evaluations/baseline-0031-control-v3-attempt-001.json"
            ).read_text(encoding="utf-8")
        )
