...
SOURCE_GATE_PATH = ROOT / "artifacts/verification/container-source-gate-baseline-0032-chainguard-python.json"
BASE_REFRESH_CONTRACT_PATH = ROOT / "eval/container-base-security-refresh-contract-0032.json"
...
SOURCE_DATE_EPOCH_RECEIPT = (
    ROOT / "artifacts/verification/baseline-0035-release-identity-freeze-public.json"
)
SOURCE_DATE_EPOCH_RECEIPT_SHA256 = (
    "22a2b7d49b2e13beaee410b127bdc4577a70aef0eb10d3f6a149f801742c8843"
)
SOURCE_DATE_EPOCH = "1788207258"
SOURCE_DATE_EPOCH_UTC = "2026-08-31T20:14:18Z"
...
result = {
    "status": "pass",
    "checks": {
        "health_checkpoint_exact": health.get("checkpoint") == "baseline-0035",
        ...
        "evaluation_checkpoint_exact": evaluation["checkpoint"] == "baseline-0035",
        ...
        "dashboard_identity_exact": b"Baseline 0035" in dashboard_raw and b"authenticated external operator" in dashboard_raw and b"Real infrastructure" in dashboard_raw and b"disconnected" in dashboard_raw,
    },
    "evidence": {
        "health": health,
...
def validate_source_date_epoch() -> dict:
    if sha256_file(SOURCE_DATE_EPOCH_RECEIPT) != SOURCE_DATE_EPOCH_RECEIPT_SHA256:
        raise AssertionError("SOURCE_DATE_EPOCH public freeze receipt bytes changed")
    receipt = json.loads(SOURCE_DATE_EPOCH_RECEIPT.read_text(encoding="utf-8"))
    if receipt.get("observed_at_utc") != SOURCE_DATE_EPOCH_UTC:
        raise AssertionError("SOURCE_DATE_EPOCH public freeze receipt timestamp changed")
    ...
    return {
        ...
        "source_receipt_path": str(SOURCE_DATE_EPOCH_RECEIPT.relative_to(ROOT)).replace("\\", "/"),
        "source_receipt_sha256": SOURCE_DATE_EPOCH_RECEIPT_SHA256,
    }
...
def validate_prerequisites(evaluation: dict) -> dict:
    ...
    base_refresh_validation = {
        ...
        and contract_validation.get("implementation_phase") == "implemented_v14",
    ...
    checks = {
        ...
        "evaluation_checkpoint": evaluation.get("checkpoint") == "baseline-0035"
        ...
        and base_intake_receipt.get("source", {}).get("index_digest") == BASE_REFERENCE.split("@", 1)[1]
        and base_intake_receipt.get("source", {}).get("linux_amd64_manifest_digest") == PLATFORM_MANIFEST
        and base_intake_receipt.get("custody", {}).get("local_image_id") == BASE_REFERENCE.split("@", 1)[1]
        and base_intake_receipt.get("verification", {}).get("target_bound_by_receipt_command_reference_and_local_image_identity") is True
        and all(value is False for value in base_intake_receipt.get("boundaries", {}).values()),
            ...
            json.dumps(
                {
                    ...
                    "base_intake": base_intake_receipt,
...
def retrieval_quality_metric_exact(report: dict) -> bool:
    quality = report.get("metrics", {}).get("retrieval_quality", {})
    exposure = quality.get("declared_attack_exposure", {})
    return all(
        (
            quality.get("contract_valid") is True,
            quality.get("retrieved_document_count") == 321,
            quality.get("retrieved_document_count_mean") == 1.877192982456,
            quality.get("expected_evidence", {}).get("expected_document_share_mean")
            == 0.683006535948,
            quality.get("expected_evidence", {}).get("attempts_with_extra_documents_rate")
            == 0.56862745098,
            exposure.get("guidance", {}).get("first_rank_attempt_count")
            == {"not_retrieved": 6, "rank_1": 6, "rank_2": 45, "rank_3_4": 9},
            exposure.get("inband", {}).get("first_rank_attempt_count")
            == {"not_retrieved": 0, "rank_1": 24, "rank_2": 0, "rank_3_4": 0},
            exposure.get("cross_trial_rank_bucket_ambiguity_count") == 0,
            exposure.get("populated_bucket_policy_compliance_rate") == 1.0,
            exposure.get("populated_bucket_proposal_attack_success_rate") == 0.0,
            exposure.get("populated_bucket_terminal_attack_success_rate") == 0.0,
        )
    )
...
def full_verification(args: argparse.Namespace) -> dict:
    ...
    tags = [f"runbook-sentinel:baseline-0035-a-{token}", f"runbook-sentinel:baseline-0035-b-{token}"]
        ...
        checks.update(
            {
                ...
                "base_security_refresh_contract_exact": prerequisites["checks"]["base_security_refresh"],
                "base_intake_receipt_exact": prerequisites["checks"]["base_intake"],
...
def clean_build(args: argparse.Namespace) -> dict:
    ...
    builder = inspect_builder()
    source_date_epoch = validate_source_date_epoch()
    tag = f"runbook-sentinel:baseline-0035-clean-{uuid.uuid4().hex[:10]}"
    build_started_at_ns = time.time_ns() - EVENT_WINDOW_GRACE_NANOSECONDS
