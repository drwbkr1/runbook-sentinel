...
$stdoutPath = Join-Path (Get-Location) 'artifacts\runtime\live-api-baseline-0027-stdout.log'
$stderrPath = Join-Path (Get-Location) 'artifacts\runtime\live-api-baseline-0027-stderr.log'
$databasePath = Join-Path (Get-Location) 'var\live-api-baseline-0027.db'
$tracePath = Join-Path (Get-Location) 'artifacts\runtime\live-api-baseline-0027-traces.jsonl'
...
$serverArgs = @(
    ...
    '--host', '127.0.0.1',
    '--port', '8877',
    '--db', 'var\live-api-baseline-0027.db',
    '--trace', 'artifacts\runtime\live-api-baseline-0027-traces.jsonl',
...
try {
    ...
    $screenshotPath = Join-Path (Get-Location) 'artifacts\verification\dashboard-baseline-0027.png'
    & $edge `
        --headless `
        --disable-gpu `
        --hide-scrollbars `
    ...
    $traceText = Get-Content -Raw 'artifacts\runtime\live-api-baseline-0027-traces.jsonl'
    ...
    $checks = [ordered]@{
        ...
        checkpoint_exact = $verification.checkpoint -eq 'baseline-0027'
        ...
        evaluation_checkpoint_exact = $verification.evaluation_checkpoint -eq 'baseline-0027'
