...
$stdoutPath = Join-Path (Get-Location) 'artifacts\runtime\live-api-baseline-0022-stdout.log'
$stderrPath = Join-Path (Get-Location) 'artifacts\runtime\live-api-baseline-0022-stderr.log'
$databasePath = Join-Path (Get-Location) 'var\live-api-baseline-0022.db'
$tracePath = Join-Path (Get-Location) 'artifacts\runtime\live-api-baseline-0022-traces.jsonl'
...
$serverArgs = @(
    ...
    '--host', '127.0.0.1',
    '--port', '8877',
    '--db', 'var\live-api-baseline-0022.db',
    '--trace', 'artifacts\runtime\live-api-baseline-0022-traces.jsonl',
...
try {
    ...
    $screenshotPath = Join-Path (Get-Location) 'artifacts\verification\dashboard-baseline-0022.png'
    & $edge `
        --headless `
        --disable-gpu `
        --hide-scrollbars `
    ...
    $traceText = Get-Content -Raw 'artifacts\runtime\live-api-baseline-0022-traces.jsonl'
    ...
    $checks = [ordered]@{
        ...
        checkpoint_exact = $verification.checkpoint -eq 'baseline-0022'
        ...
        evaluation_checkpoint_exact = $verification.evaluation_checkpoint -eq 'baseline-0022'
        ...
        dashboard_baseline_exact = [bool]$verification.dashboard_baseline_0022
