...
def main() -> None:
    ...
    if contract.get("schema_version") != "1.3":
        errors.append("terminal-state contract schema must be 1.3")
    if contract.get("contract_id") != "synthetic-terminal-state-v4":
        errors.append("terminal-state contract ID must be synthetic-terminal-state-v4")
    if contract.get("idempotency_key_template") != "baseline-0008:{scenario_id}:{trial}":
        errors.append("idempotency template must bind baseline-0008")
    if execute_count != 9 or no_execute_count != 17:
        errors.append("terminal-state contract must contain 9 actionable and 17 no-action cases")
