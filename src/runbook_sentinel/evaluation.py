...
def _adversarial_topology_split_coverage(
    scenarios: list[dict], contract: dict
) -> dict:
    errors: list[str] = []
    if not isinstance(contract, dict):
        contract = {}
    ...
    required_domains = contract.get("required_domains", [])
    required_splits = contract.get("required_splits", [])
    minimum = contract.get("minimum_cases_per_adversarial_domain_split")
    if (
        not isinstance(required_domains, list)
        or set(required_domains) != set(REQUIRED_DOMAINS)
        or len(required_domains) != len(REQUIRED_DOMAINS)
    ):
        errors.append("required_domains")
    ...
    if minimum != 1 or isinstance(minimum, bool):
        errors.append("minimum_cases_per_adversarial_domain_split")
    ...
    for scenario in scenarios:
        ...
        split = scenario.get("split")
        if domain not in counts:
            errors.append(f"{scenario_id}:domain")
            continue
        if split not in counts[domain]:
            errors.append(f"{scenario_id}:split")
            continue
    ...
    split_coverage = {
        split: (
            sum(counts[domain][split] >= threshold for domain in required_domains)
            / len(required_domains)
            if required_domains
            else 0.0
        )
    ...
    return {
        "adversarial_topology_split_contract_id": contract.get("contract_id"),
        "adversarial_topology_split_contract_valid": not errors,
        "adversarial_topology_split_contract_errors": sorted(set(errors)),
        "required_adversarial_domain_split_pair_count": pair_count,
        "covered_adversarial_domain_split_pair_count": covered_pair_count,
        "minimum_cases_per_adversarial_domain_split": minimum,
...
def _adversarial_outcome_split_coverage(
    scenarios: list[dict], terminal_contract: dict, contract: dict
) -> dict:
    errors: list[str] = []
    if not isinstance(contract, dict):
    ...
    required_splits = contract.get("required_splits", [])
    minimum = contract.get("minimum_cases_per_adversarial_outcome_split")
    expected_outcomes = ["abstain", "diagnose", "propose_action", "request_evidence"]
    if required_outcomes != expected_outcomes:
        errors.append("required_outcomes")
        required_outcomes = expected_outcomes
    if required_splits != ["development", "test"]:
        errors.append("required_splits")
        required_splits = ["development", "test"]
    if minimum != 1 or isinstance(minimum, bool):
        errors.append("minimum_cases_per_adversarial_outcome_split")
    threshold = minimum if isinstance(minimum, int) and not isinstance(minimum, bool) else 1

    counts = {
        outcome: {split: 0 for split in required_splits}
    ...
    for scenario in scenarios:
        ...
        if outcome not in counts:
            errors.append(f"{scenario_id}:outcome")
            continue
        if split not in counts[outcome]:
            errors.append(f"{scenario_id}:split")
            continue
        terminal = terminal_states.get(scenario_id)
        if not isinstance(terminal, dict):
        ...
        if outcome == "propose_action":
            if (
                ...
                or terminal.get("execute") is not True
                or terminal.get("action") != expected_action
            ):
                errors.append(f"{scenario_id}:terminal_outcome")
                continue
        elif (
            expected_action is not None
            or terminal.get("execute") is not False
            or terminal.get("action") is not None
        ):
            errors.append(f"{scenario_id}:terminal_outcome")
            continue
    ...
    pair_count = len(required_outcomes) * len(required_splits)
    covered_pair_count = pair_count - len(missing_pairs)
    split_coverage = {
        split: (
            sum(counts[outcome][split] >= threshold for outcome in required_outcomes)
    ...
    return {
        "adversarial_outcome_split_contract_id": contract.get("contract_id"),
        "adversarial_outcome_split_contract_valid": not errors,
        "adversarial_outcome_split_contract_errors": sorted(set(errors)),
        "required_adversarial_outcome_split_pair_count": pair_count,
        "covered_adversarial_outcome_split_pair_count": covered_pair_count,
        "minimum_cases_per_adversarial_outcome_split": minimum,
...
def _adversarial_domain_outcome_split_coverage(
    scenarios: list[dict], terminal_contract: dict, contract: dict
) -> dict:
    errors: list[str] = []
    if not isinstance(contract, dict):
    ...
    required_pairs = contract.get("required_domain_outcome_pairs", [])
    required_splits = contract.get("required_splits", [])
    minimum = contract.get("minimum_cases_per_adversarial_domain_outcome_split")
    if required_pairs != expected_pairs:
        errors.append("required_domain_outcome_pairs")
        required_pairs = expected_pairs
    if required_splits != ["development", "test"]:
        errors.append("required_splits")
        required_splits = ["development", "test"]
    if minimum != 1 or isinstance(minimum, bool):
        errors.append("minimum_cases_per_adversarial_domain_outcome_split")
    threshold = minimum if isinstance(minimum, int) and not isinstance(minimum, bool) else 1

    required_pair_tuples = {
        (pair["domain"], pair["outcome"]) for pair in required_pairs
    }
    counts: dict[str, dict[str, dict[str, int]]] = {}
    for pair in required_pairs:
    ...
    for scenario in scenarios:
        ...
        if split not in required_splits:
            errors.append(f"{scenario_id}:split")
            continue
        if (domain, outcome) not in required_pair_tuples:
            errors.append(f"{scenario_id}:domain_outcome:{domain}:{outcome}")
            continue
        terminal = terminal_states.get(scenario_id)
        if not isinstance(terminal, dict):
            errors.append(f"{scenario_id}:terminal_missing")
    ...
    missing_cells = [
        ...
        for split in required_splits
        for pair in required_pairs
        if counts[pair["domain"]][pair["outcome"]][split] < threshold
    ]
    cell_count = len(required_pairs) * len(required_splits)
    covered_cell_count = cell_count - len(missing_cells)
    split_coverage = {
        split: (
            sum(
                counts[pair["domain"]][pair["outcome"]][split] >= threshold
                for pair in required_pairs
            )
            / len(required_pairs)
            if required_pairs
    ...
    return {
        "adversarial_domain_outcome_split_contract_id": contract.get(
            "contract_id"
        ),
        "adversarial_domain_outcome_split_contract_valid": not errors,
        "adversarial_domain_outcome_split_contract_errors": sorted(set(errors)),
        "required_adversarial_domain_outcome_pair_count": len(required_pairs),
        "required_adversarial_domain_outcome_split_cell_count": cell_count,
        "covered_adversarial_domain_outcome_split_cell_count": covered_cell_count,
        "minimum_cases_per_adversarial_domain_outcome_split": minimum,
        "case_count_by_adversarial_domain_outcome_split": counts,
        "missing_adversarial_domain_outcome_split_cells": missing_cells,
        "adversarial_domain_outcome_split_coverage": (
            covered_cell_count / cell_count if cell_count else 0.0
        ),
        "split_adversarial_domain_outcome_coverage": split_coverage,
    }
...
def _action_split_coverage(
    ...
    required_actions = contract.get("required_actions", [])
    required_splits = contract.get("required_splits", [])
    minimum = contract.get("minimum_cases_per_action_split")
    if set(required_actions) != set(ACTION_SPECS) or len(required_actions) != len(ACTION_SPECS):
        errors.append("required_actions")
    if required_splits != ["development", "test"]:
        errors.append("required_splits")
    if minimum != 1:
        errors.append("minimum_cases_per_action_split")
    ...
    return {
        "action_split_contract_id": contract.get("contract_id"),
        "action_split_contract_valid": not errors,
        "action_split_contract_errors": sorted(set(errors)),
        "required_action_split_pair_count": pair_count,
        "covered_action_split_pair_count": covered_pair_count,
        "minimum_cases_per_action_split": minimum,
...
def run_evaluation(
    ...
    action_split_contract = catalog["action_split_coverage_contract"]
    ...
    action_split_coverage = _action_split_coverage(
        scenarios, terminal_contract, action_split_contract
    )
    ...
    gates = {
        ...
        == 1.0,
        "action_split_contract_valid": metrics["coverage"][
            "action_split_contract_valid"
        ],
        "action_split_coverage_is_one": metrics["coverage"][
            "action_split_coverage"
        ]
        == 1.0,
        "development_action_split_coverage_is_one": metrics["coverage"][
            "split_action_coverage"
        ].get("development")
        == 1.0,
        "test_action_split_coverage_is_one": metrics["coverage"][
            "split_action_coverage"
        ].get("test")
        == 1.0,
        "adversarial_topology_split_contract_valid": metrics["coverage"][
            "adversarial_topology_split_contract_valid"
        ],
        "adversarial_topology_split_coverage_is_one": metrics["coverage"][
            "adversarial_topology_split_coverage"
        ]
        == 1.0,
        "development_adversarial_topology_split_coverage_is_one": metrics[
            "coverage"
        ]["split_adversarial_topology_coverage"].get("development")
        == 1.0,
        ...
        == 1.0,
        "adversarial_outcome_split_contract_valid": metrics["coverage"][
            "adversarial_outcome_split_contract_valid"
        ],
        "adversarial_outcome_split_coverage_is_one": metrics["coverage"][
            "adversarial_outcome_split_coverage"
        ]
        == 1.0,
        "development_adversarial_outcome_split_coverage_is_one": metrics[
            "coverage"
        ]["split_adversarial_outcome_coverage"].get("development")
        == 1.0,
        ...
        == 1.0,
        "adversarial_domain_outcome_split_contract_valid": metrics["coverage"][
            "adversarial_domain_outcome_split_contract_valid"
        ],
        "adversarial_domain_outcome_split_coverage_is_one": metrics["coverage"][
            "adversarial_domain_outcome_split_coverage"
        ]
        == 1.0,
        "development_adversarial_domain_outcome_split_coverage_is_one": metrics[
            "coverage"
        ]["split_adversarial_domain_outcome_coverage"].get("development")
        == 1.0,
        ...
        "baseline_disposition": (
            ...
            and metrics["coverage"]["action_split_contract_valid"]
            and metrics["coverage"]["action_split_coverage"] == 1.0
            and metrics["coverage"]["split_action_coverage"].get("development")
            == 1.0
            and metrics["coverage"]["split_action_coverage"].get("test") == 1.0
            and metrics["coverage"]["adversarial_topology_split_contract_valid"]
            and metrics["coverage"]["adversarial_topology_split_coverage"] == 1.0
            and metrics["coverage"]["split_adversarial_topology_coverage"].get(
                "development"
            )
            == 1.0
            and metrics["coverage"]["split_adversarial_topology_coverage"].get(
            ...
            == 1.0
            and metrics["coverage"]["adversarial_outcome_split_contract_valid"]
            and metrics["coverage"]["adversarial_outcome_split_coverage"] == 1.0
            and metrics["coverage"]["split_adversarial_outcome_coverage"].get(
                "development"
            )
            == 1.0
            and metrics["coverage"]["split_adversarial_outcome_coverage"].get(
