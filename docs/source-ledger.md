...
BASELINE-0035 admits only citation and narrow paraphrase from exact primary RAGChecker, RAGAs, CheckList, and BERGEN publisher records under `artifacts/verification/research-source-gate-baseline-0035-evaluation-contract-integrity.json`. The approved principles are fine-grained module diagnostics, capability-specific tests beyond aggregates, and reproducible configuration comparisons. Runbook Sentinel's semantic categories, safe-superset proof, mutations, classifier, and synthetic evidence are project-authored. No external paper bytes, code, data, benchmark, model, prompt, judge, metric package, dependency, service, secret, paid asset, or real infrastructure is imported.
