...
Fresh public v0.0.14 source and package verification passes every existing gate, but the real loopback approval endpoint accepts any nonblank caller-declared `actor`. A caller naming itself `sentinel-agent-self-declared` received an approval token and completed a synthetic restart with verified postconditions; persisted state, audit, and traces record the string as the approver. The model and MCP still have no approval tool or credential, but the rendered `human approval` claim is stronger than the enforced boundary. Current primary-source review excludes direct Bearer use because RFC 6750 requires TLS and excludes OAuth as disproportionate architecture; RFC 9110 supplies only generic HTTP authentication semantics. The user explicitly approved the recommended no-cost, project-specific per-launch operator capability. The exact decision is locked and reconciled, ADR 0011 is accepted, and `operator-authentication-v1` freezes ten ordered development and held-out cases before implementation. The candidate must derive identity server-side, keep the capability outside the agent, MCP, logs, traces, repository, package, persistence, arguments, and environment, and label the boundary `authenticated external operator`, not proof of human presence. Baseline 0015 now proceeds at UNIT-003.
