...
Fresh public v0.0.14 source and package verification passes every existing gate, but the real loopback approval endpoint accepts any nonblank caller-declared `actor`. A caller naming itself `sentinel-agent-self-declared` received an approval token and completed a synthetic restart with verified postconditions; persisted state, audit, and traces record the string as the approver. The model and MCP still have no approval tool or credential, but the rendered `human approval` claim is stronger than the enforced boundary. Current primary-source review excludes direct Bearer use because RFC 6750 requires TLS and excludes OAuth as disproportionate architecture; RFC 9110 supplies only generic HTTP authentication semantics. The recommended bounded option is a no-cost, project-specific per-launch operator capability using already-available standard-library primitives, supplied outside the agent and used to derive the operator identity server-side. It must be unavailable to the model, MCP, logs, traces, repository, package, and persisted state and must be labeled `authenticated external operator`, not proof of human presence. That option still requires a new secret/access-control choice; a label-only correction would be truthful without preventing arbitrary local approval. Baseline 0015 is paused at that explicit architecture gate before contract freeze or implementation.
