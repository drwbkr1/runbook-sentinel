...
The verified public v0.0.16 package externally anchors completed evaluation traces, but its real API and MCP trace files persist only their internal unkeyed chain. A non-destructive public-package probe shows that a structurally valid suffix deletion remains a valid unanchored live chain, while the omitted full endpoint detects exact count and digest mismatch. Baseline 0017 freezes ten development and held-out cases for empty start, exact first write, tail truncation, anchor mutation, missing and orphaned files, extra suffix, exact restart/resume, malformed JSON, and wrong trace-name binding. The bounded candidate may add only a deterministic sibling anchor, durable flush/fsync ordering, same-directory secure temporary creation, atomic replacement, and fail-closed resume validation. It adds no key, signer, credential, dependency, service, collector, real-infrastructure connection, or authenticity claim.
