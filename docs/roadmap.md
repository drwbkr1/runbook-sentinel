...
Fresh public v0.0.14 source and package verification passes every existing gate, but the real loopback approval endpoint accepts any nonblank caller-declared `actor`. A caller naming itself `sentinel-agent-self-declared` received an approval token and completed a synthetic restart with verified postconditions; persisted state, audit, and traces record the string as the approver. The model and MCP still have no approval tool or credential, but the rendered `human approval` claim is stronger than the enforced boundary. An authenticated local operator capability would materially strengthen identity but requires a new secret or access-control choice; a label-only correction would be truthful without preventing arbitrary local approval. Baseline 0015 is paused at that explicit architecture gate before contract freeze or implementation.
