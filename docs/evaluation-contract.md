...
- Predecessor lifecycle is a prerequisite gate. `retrieval-predecessor-successor-lifecycle-v1.2` preserves the released 4,643-byte retrieval identity and permits a later source only at exact precomputed v5 SHA-256 `f5514cb7...`, with a public bridge receipt, v4/v5 development equivalence, v3 default before selection, no held-out bridge access, and fail-closed benchmark/comparison presence. The bridge changes no product runtime path and cannot rewrite BASELINE-0031/0033 evidence. The BASELINE-0033 release verifier remains byte-exact and verifies its frozen fixture; only two evaluation validators may recognize the later current-tree identity. Its release test may move from exact public 6,941-byte identity `3000e914...` to only the precomputed 6,944-byte identity `861d8f34...`, replacing the current phase variable with literal `frozen`; unknown bytes fail closed.
...
- Lifecycle schema 1.2 retains three exact states: no implementation/no result, implementation present/result absent, and implementation plus result. The middle state is required for a public implementation seal. Any other presence combination or explicit phase mismatch fails closed. Schema 1.2 also binds the frozen release-fixture phase correction without changing admissibility, selection, runtime, or security rules.
- Schema 1.2 revision 3 adds a meta-test lifecycle gate after the first exact release-test implementation exposes a preimplementation-only assertion. The meta-test may remain at 2,741-byte SHA-256 `b4f2b660...` or move only to precomputed phase-aware 3,159-byte `ee7c67c2...`; unknown bytes fail closed. The release-test correction remains separately bound at `3000e914...` or `861d8f34...`.
- The revision-3 helper addendum binds the proof helper at 2,708-byte `fe1e1d9e...` or phase-aware 3,029-byte `a9f499dc...`. The meta-test and helper may change together only after this addendum is public and reconciled; the independent lifecycle validator rejects every unknown identity without adding another self-referential test.
- The successor-phase test addendum keeps schema revision 3 and binds three current-or-future test-only identity pairs. Future hashes are 3,449-byte `ee640f79...`, 4,658-byte `09e13990...`, and 3,656-byte `b50247ed...`; their source must recognize only the six governed phases and corrupt the active retrieval identity in fail-closed tests. Runtime, results, default, admissibility, selection, authorization, and security rules do not change.
...
- The authoritative 107-file pre-reveal manifest SHA-256 is `fc1b5e756ba0f06613c0996b50aa6f4ffcd4ebae5749988ebb1095c21aa27e05`; earlier `cc492319...` and `7c510e2e...` manifests are retained as superseded and were never used for a candidate. No immutable candidate may run before this manifest commit is public.
...
- The admissible candidate and final-manifest source/package runs pass all 126 attempts and exact 216-event trace checks. A remote-only clone independently repeats 25 validators, 41 tests, source/package evaluation, archive identity, bounded MCP, real runtime surfaces, scans, and original-detail dashboard review.
