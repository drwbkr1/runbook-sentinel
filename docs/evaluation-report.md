...
Retained limitations: six-cell synthetic coverage is not universal prompt-injection resistance; the older adversarial topology metric remains scenario-label coverage and one development API hostile runbook is not selected by retrieval; 41 cases do not satisfy the separate at-least-48-case v0.1.0 target; no real infrastructure or Docker claim is made. Review, merge, tag, assets, downloads, rendered public pages, and public-tag verification remain pending.
