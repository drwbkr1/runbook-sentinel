...
| Caller forges approver identity or self-approves | Measured open gap in public v0.0.14: approval creation accepts any nonblank caller-declared actor and the dashboard calls the boundary human approval. Token hashing, action binding, expiry, replay, idempotency, postconditions, model isolation, and MCP isolation still hold, but approver identity is not authenticated. Baseline 0015 requires an explicit credential/access architecture decision before claiming a control. |
