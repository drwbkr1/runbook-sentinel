...
| Caller forges approver identity or self-approves | Measured open gap in public v0.0.14: approval creation accepts any nonblank caller-declared actor and the dashboard calls the boundary human approval. The approved baseline-0015 contract requires exactly one project-specific per-launch `Sentinel-Capability` credential before body parsing, rejects caller `actor`, derives a launch-scoped identity server-side, and calls the boundary authenticated external operator. Direct Bearer/OAuth use remains excluded. This control is frozen but not yet implemented. |
