...
| Persisted telemetry is changed, reordered, deleted, duplicated, malformed, or truncated after emission | Every event has a canonical digest, contiguous sequence, and predecessor digest; append verifies the existing prefix; completed evaluations bind exact event count and final digest. Public v0.0.16 live API/MCP traces still lack a persisted endpoint, so valid suffix loss remains measurable; BASELINE-0017 freezes sibling live-anchor and fail-closed resume requirements before implementation. The unkeyed design does not authenticate a writer or resist full chain-and-anchor recomputation. |
