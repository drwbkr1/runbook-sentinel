...
| Caller forges approver identity or self-approves | Measured open gap in public v0.0.14: approval creation accepts any nonblank caller-declared actor and the dashboard calls the boundary human approval. Token hashing, action binding, expiry, replay, idempotency, postconditions, model isolation, and MCP isolation still hold, but approver identity is not authenticated. Direct Bearer use is excluded because the current HTTP surface lacks RFC 6750's mandatory TLS; OAuth is unnecessary for the bounded local use. A project-specific per-launch capability is feasible but remains unimplemented behind the explicit secret/access gate. |
